...
public class LackeySmallAI : AIntelligence
{
    ...
    private AIMode mode = AIMode.StrategyMaking;
    ...
    [SerializeField] private bool IsCrazy = false;

    [SerializeField] private bool StandStill = false;

    private Vector2 preferedDirection = Vector2.left;
...
/*        else if (mode == AIMode.SuperAttack)
        ...
        else if (mode == AIMode.RunFarAway)
        {
            inpInf = RunFarAway();
        }
        else
    ...
    {
        if (SearchAim() != null || modeTimeLeft <= 0)
        {
    ...
    private InputInfo StrategyMaking()
    {
        ...
        if (aim == null)
        {
            ...
            {
                modeTimeLeft = 1f;
                preferedDirection = new Vector2();
                mode = AIMode.AimSearch;
                return CopyLastInputRoughly();
            }
            if (!body.HaveItem && Random.value > 0.7)
            {
                mode = AIMode.ItemSearch;
            }
            else
            {
                modeTimeLeft = 0.5f + Random.value;
                preferedDirection = eye.GetFreeDirection(3);
                mode = AIMode.AimSearch;
            }
            return CopyLastInputRoughly();
        }
        ...
        var dist = aimDist;
        if (aim.Dead || !aim.CanBeHit && Random.value > 0.2)
        {
            modeTimeLeft = 3;
            preferedDirection = eye.GetFreeDirection(3);
            mode = AIMode.AimSearch;
        ...
        else if (dist < retreatRadius && Random.value > 0.5)
        {
            modeTimeLeft = 0.5f;
            mode = AIMode.Retreat;
        }
        else if (body.AlmostPassedOut && Random.value > 0.6)
        {
            modeTimeLeft = 4f;
            preferedDirection = eye.GetFreeDirection(1) * 0.3f + aimDir.normalized * 0.7f;
            mode = AIMode.RunFarAway;
        }
