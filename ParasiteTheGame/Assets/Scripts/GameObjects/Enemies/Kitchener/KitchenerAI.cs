...
public class KitchenerAI : AIntelligence
{
    Kitchener kitchener;
    ...
    void Start()
    {
        kitchener = GetComponent<Kitchener>();
        eye = GetComponent<AllSeeingEye>();
        var direction = new Vector2(Random.value * 2 - 1, Random.value * 2 - 1).normalized;
        previousInput = new InputInfo(new Vector2(Random.Range(-1, 2), Random.Range(-1, 2)), direction, false, !kitchener.HaveItem, true, false);
    }
    ...
    void Update()
    {
        ...
        var direction = new Vector2(Random.value * 2 - 1, Random.value * 2 - 1).normalized;
        var inpInf = new InputInfo(new Vector2(Random.Range(-1, 2), Random.Range(-1, 2)), direction, false, !kitchener.HaveItem, true, false);
        ...
        kitchener.ControlledUpdate(inpInf);
    }
    ...
    private InputInfo StrategyMaking()
    {
        ...
        else if (!kitchener.HaveItem && Random.value > 0.7)
        {
            mode = AIMode.ItemSearch;
        }
        else if (kitchener.AlmostPassedOut && Random.value > 0.5 && dist > kitchener.GetUserRadius())
        {
    ...
    {
        if (kitchener.GetDamageType() == DamageType.Distant)
        {
            modeTimeLeft = 1f;
    ...
    private InputInfo MeleeAttack()
    {
        ...
        return new InputInfo(walkDirection, aimDirection, false, !kitchener.HaveItem, true, false);
    }
    ...
    {
        if (modeTimeLeft <= 0 || kitchener.HaveItem)
        {
            modeTimeLeft = 0;
            mode = AIMode.StrategyMaking;
        }

        return new InputInfo(ItemDir.normalized, ItemDir.normalized, false, !kitchener.HaveItem, false, false);
    }
